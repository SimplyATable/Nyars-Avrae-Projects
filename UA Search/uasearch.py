...
{{(f' -title "{s.name}" -desc "" -f "Description|{s.desc}" -footer "Type: {s.type}"') if 'name' in s else f'-f "List of Items containing {n}|{raw if s else "No Matches Found"}"' if n else '-desc "`!testua [search]` to look up something.\n[Source](https://bit.ly/3u73Iu7)"'}}
